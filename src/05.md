...
export default {
 ...
 setup() {
   ...
   onMounted(() => {
    // 當生命週期onMounted的時候就能看到，DOＭ已被getDom存取
     console.log("getDom -->", getDom);
   });

   // 並return出去，在模板中需要定義ref="getDom"同名的屬性
   return { getDom };
 },
};
```

### reactive : 與 ref()以外，可以包裝響應式的<span style="color:red;">reactive</span>函式，但是<span style="color:red;">reactive</span>函式只能包裝<span style="color:red;"> 物件、陣列 </span>，使用上不需要加上.value，就如取得物件實字的 value 值一樣。
