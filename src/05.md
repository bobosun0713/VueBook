...
2. props 是響應式proxy對象， 如果在 setup()函式中解構出來，會消除 props 響應式特性，如果使用解構方式取得props資料，需使用[toRefs]()操作來重新綁定響應
...
使用解構時，需使用toRefs()方法重新綁定響應式，但使用的方式需加上.value
```js
...
<template>
  <TestComponent ref="getDom"></TestComponent>
</template>
```
