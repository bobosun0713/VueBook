...
在使用 Vue 框架來說其實蠻彈性的，可使用 CDN 或者下載 JS 直接引入外，也可以使用官方的 Vue-CLI 或 Vite(新推出腳手架)來使用（這邊以 Vite v3 做開發）
...
> [!note]
> 這邊可能會注意到 npm create 這段指令，其實 npm create 是 npm init 的別名，可以打 npm init --help 就能知道 npm create 與 npm init 關西，只是官網沒有特別著名 npm create 是 npm init 的別名，而 init 是規範的形式，所以使用哪個指令並不重要！
<!--endsec-->
...
> [!note]
> 在安裝套件時會發現到 有些副檔名會生成的副檔名 (mjs、cjs) ，這部分依照 vite 官方的解釋，會根據套件的類型來輸出有效的副檔名
> 題外話: vite v3 只支援 ESM 的方式，而 vite 有一個預編譯功能，會對於套件不是 ESM 規範的套件再轉化為 ES Module，所以才會看到 package 裡 type 為 module 但卻能讀到 .cjs( CommonJS )的檔案 ，這些 vite 默認會幫忙處理
...
> [!note]
> 如果安裝的方式是使用 Vue 官方的安裝包 (npm init vue@latest) 時，如果也順便安裝 ESLint 時，記得規則需要加上要再 ESLint 設定檔，設置 env:{node:true}，否則 postcss.config 設定檔會出現 'module' is not defined 錯誤訊息 (錯誤規則：no-undef)
...
- 使用者與工作區設定，在編輯器中(Vscode)中，可透過 settings.json 來設定我們的編輯器，而 settings.json 可以多個，可以依照 settings.json 檔案位置，編輯器再決定優先度！
   ...
   4. 在專案設定 .settings.json 確保專案夥伴的編輯器設定是否都一樣 
      ```json 
      {
...
## 使用 pnpm 管理套件安裝以上項目
  安裝流程會依照上面流程走，只是替換為 pnpm 語法 [pnpm 官網](https://pnpm.io/)

 <!--sec data-title="pnpm 啟動 vite + vue3 專案 :" data-id="section1" data-show=true ces-->
  1. 啟動 vite + vue3 專案
     ```
     // create 為 啟動套件 或 創建項目
     $ pnpm create vite@latest
     $ pnpm install 
     ```

  2. 安裝  ESLint、Prettier : [pnpm dlx](https://pnpm.io/cli/dlx)
     ```
     // 安裝 ESLint ，這邊需注意使用 npm 安裝時會採用 npx 的方式啟用 ESLint CLI，而使用 pnpm 安裝時，會採用 pnpm dlx
  ...
  3. 導入 Tailwindcss 時
     ...
     // 產生 tailwind.config.cjs 跟 postcss.config.cjs 檔案
     pnpm dlx tailwindcss init -p
     ```

  4. 以上使用 pnpm 的方式，建構一個 vite 專案，啟動專案時輸入 pnpm run dev
     > pnpm run 也有別名，去掉 run 就可以執行腳本 pnpm dev，但需要注意只適用於不會與 pnpm 命令相同的名子！
 <!--endsec-->
